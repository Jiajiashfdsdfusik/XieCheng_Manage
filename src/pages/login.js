...
const LoginItem = (props) => {
    ...
    .then(json => {
        ...
        if(json.login === 'login'){
          console.log('登录成功,即将跳转!');
          props.action({login: 'login', name: username, power: json.power});
          navigate('/manage');
          return;
        }else{
          // if(json.error === '密码错误'){
    ...
    return (
      <div className='content_login'>
        <Modal title="登录错误" open={isModalOpen} onOk={() => setIsModalOpen(false)} onCancel={() => setIsModalOpen(false)}>
          <p>{error}</p>
        </Modal>
          <Form className='content_form'
              ...
              autoComplete="off"
          >
            <h2 className='content_title'>管理员登录</h2>
            <Form.Item
              label="Username"
              name="username"
